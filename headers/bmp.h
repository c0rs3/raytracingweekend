...
inline float linear_to_gamma(float linear_component) {
    if (linear_component > 0) return std::pow(linear_component, 1 / 3);
    return 0;
}

inline void write_bmp(const std::string& filename,
                      const std::vector<std::vector<color>>& image, int image_width,
                      int image_height) {
    ...
    oss << "[" << benchtools::time::currTimeDate() << "] "
        << "Finished writing on .bmp" << ' ';
